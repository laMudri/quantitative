...
\begin{document}
  ...
  \begin{frame}
    ...
    \begin{itemize}
      % \item<1-> A well scoped substitution $\sigma$ maps variables $x$ to
      %   synthesising terms $e_x$.
    \item<1-> Well scoped substitution
      \begin{flalign*}
        m \Rightarrow n :\equiv \prod_{x \in n}{\mathrm{Tm}~m~\mathrm{syn}}
      \end{flalign*}
      % \item $\typed \sigma : \Gamma_m \Rightarrow_\sigma \Gamma_n$
  ...
  \end{frame}
  \begin{frame}
    \frametitle{Relational semantics}
  \end{frame}
\end{document}
