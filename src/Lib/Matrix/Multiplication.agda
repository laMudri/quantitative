  ...
  *-indic fff x = fst annihil x
  ...
  multM-identity = li , ri
    ...
    li {succ m , n} N (os i , k) =
      (1M *M N) (os i , k)  ≈[ refl ]≈
      indic (floor (mapDec _ _ (i ≟th oe))) * N (zeroth , k)
       + (sum λ j → indic (floor (os i ≟th o′ j)) * N (o′ j , k))
        ≈[
         (indic (floor (mapDec _ _ (i ≟th oe))) * N (zeroth , k)
            ≈[ ≡⇒≈ (≡.cong indic
              (≡.trans (floor-mapDec _ _ _)
                       (floor-true _ (oe-unique i oe)))) *-cong refl ]≈
          e1 * N (zeroth , k)  ≈[ *-identity .fst _ ]≈
          N (zeroth , k)
            ≈[ ≡⇒≈ (≡.cong (λ z → N (os z , k)) (oe-unique oe i)) ]≈
          N (os i , k)  QED)
        +-cong
         ((sum λ j → e0 * N (o′ j , k))
    ...
    ri {m , succ n} M (i , os k) =
      (M *M 1M) (i , os k)  ≈[ refl ]≈
      M (i , zeroth) * indic (floor (mapDec _ _ (oe ≟th k)))
       + (sum λ j → M (i , o′ j) * indic (floor (o′ j ≟th os k)))
        ≈[
         (M (i , zeroth) * indic (floor (mapDec _ _ (oe ≟th k)))
            ≈[ refl *-cong ≡⇒≈ (≡.cong indic
              (≡.trans (floor-mapDec _ _ _)
                       (floor-true _ (oe-unique oe k)))) ]≈
          M (i , zeroth) * e1  ≈[ *-identity .snd _ ]≈
          M (i , zeroth)
            ≈[ ≡⇒≈ (≡.cong (λ z → M (i , os z)) (oe-unique oe k)) ]≈
          M (i , os k)  QED)
        +-cong
         ((sum λ j → M (i , o′ j) * e0)
