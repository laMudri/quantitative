...
module Quantitative.Resources.Substitution
  ...
  varRCtx-leftPart :
    ∀ {m} n (th : Fin m) rho →
    varRCtx (1≤-leftPart n th) rho Δ.≈ varRCtx th rho +V 0Δ
  ...
  varRCtx-rightPart :
    ∀ m {n} (th : Fin n) rho →
    varRCtx (1≤-rightPart m th) rho Δ.≈ 0Δ {m} +V varRCtx th rho
  ...
  ... | inl thl = var a
    where
    a : Δl +V Δn +V Δm Δ.≤ varRCtx (1≤-leftPart (n +N m) thl) e1
  ...
  ... | inr thm = var a
    where
    a : Δl +V Δn +V Δm Δ.≤ varRCtx (1≤-rightPart l (1≤-rightPart n thm)) e1
    a rewrite VZip≡ (varRCtx-rightPart l (1≤-rightPart n thm) e1)
  ...
  singleSubstRes : ∀ {m Δ Δ0 Δ1 t} → Δ0 |-r t → Δ Δ.≤ e1 *Δ Δ0 +Δ Δ1 →
  ...
  liftSubstRes {Δm = Δm} {Δn} vf vfr =
    ...
    vfr' : e0 :: Δn |-r*[ Δm ] 1≤-tabulate (punchInNManyVars 1 0 o vf)
  ...
  module DecLE (_≤?_ : ∀ x y → Dec (x R.≤ y)) where
    ...
    substituteRes {n = n} {Δn = Δn} (var {th = th} sub) vf vfr = go th sub vf vfr
      ...
      go : ∀ {m} {Δm : RCtx m} (th : Fin m) →
           Δm Δ.≤ varRCtx th e1 → (vf : Subst m n) (vfr : Δn |-r*[ Δm ] 1≤-tabulate vf) →
           Δn |-r vf th
    ...
    ~~>-preservesRes {Δ = Δ} (app {Δe = Δe} {Δs} split (the (lam s0r)) s1r) (beta S T s0 s1) =
      ...
      split' : ∀ {s1} → Δs |-r s1 → Δ Δ.≤ e1 *Δ Δs +Δ Δe
