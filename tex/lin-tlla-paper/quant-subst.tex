...
and Pfenning and Davis' S4 modal type theory \cite{judgmental}.
...
\subsection{DILL}\label{sec:dill}
...
obvious rules.
\fixmeBA{should put the rules for the precise variant of DILL we are using in an appendix}
...
In constructing DILL derivations, we make regular use of the following
admissible rule, allowing intuitionistic assumptions to be turned into linear
assumptions at any point in a derivation.

\begin{lemma}[environment weakening]\label{lem:env-weak}
  In DILL, $\Gamma; A, \Delta \vdash B$ implies $\Gamma, A; \Delta \vdash B$.
\end{lemma}
\begin{proof}
  See \cite{Barber1996}, Lemma 2.5.
\end{proof}
...
\begin{proof}
  ...
  For the \TirName{$\excl{\unrestricted}{}$-I} rule, we are required to prove
  that the following is admissible in DILL.
  \[
    \inferrule*
    {\Gamma; \Delta \vdash A}
    {\Gamma, \Delta; \cdot \vdash \oc A}
  \]
  This follows from \autoref{lem:env-weak} applied to $\Delta$, followed by
  \TirName{$\oc$-I}.

  Splitting of contexts is more involved.
  We shall again take \TirName{$\tensor{}{}$-I} as our example.
  We shall assume, without loss of generality, that the inequality in the
  \TirName{$\tensor{}{}$-I} rule is actually an equality, producing the rule
  on the left below (to recover the full rule, apply environment weakening and intuitionistic weakening to the resultant DILL derivation).
  After applying the inductive hypothesis to both halves of the pair, we are
  required to prove the rule on the right admissible in DILL.
  \begin{mathpar}
    \inferrule*[right=$\tensor{}{}$-I$'$]
    {\ctx\Gamma P \vdash A \\ \ctx\Gamma Q \vdash B}
    {(\resctx P + \resctx Q)\Gamma \vdash \tensor A B}
    \and
    \inferrule*
    {\Gamma_A; \Delta_A \vdash A \\ \Gamma_B; \Delta_B \vdash B}
    {\Gamma_A \cup \Gamma_B \cup (\Delta_A \cap \Delta_B);
      (\Delta_A \cup \Delta_B) - (\Delta_A \cap \Delta_B) - \Gamma_A - \Gamma_B
      \vdash A \otimes B}
  \end{mathpar}
  To show this rule admissible in DILL, we simplify the contexts as follows:
  let $\Gamma := \Gamma_A \cup \Gamma_B \cup (\Delta_A \cap \Delta_B)$,
  $\Delta_A' := \Delta_A - \Delta_B - \Gamma_B$, and
  $\Delta_B' := \Delta_B - \Delta_A - \Gamma_A$.
  Notice that $\Delta_A'$ and $\Delta_B'$ are disjoint, and also that
  $\Delta_A' \cup \Delta_B' =
  (\Delta_A \cup \Delta_B) - (\Delta_A \cap \Delta_B) - \Gamma_A - \Gamma_B$
  (using the fact that $\Gamma_A$ and $\Delta_A$ are disjoint, as are
  $\Gamma_B$ and $\Delta_B$).
  We can prove $\Gamma; \Delta_A' \vdash A$ and $\Gamma; \Delta_B' \vdash B$
  using the two respective hypotheses and environment \& intuitionistic
  weakening, and then apply \TirName{$\otimes$-I} to get the desired conclusion.
  %We shall again take \TirName{$\tensor{}{}$-I} as our example.
  ...
  %from the premises.
  %To prove this rule admissible, on both premises we apply
  %\autoref{lem:env-weak} to all variables in $\Delta_A$ or $\Delta_B$ that are
  %also in $\Gamma$.
  %This gives us $\Gamma; \Delta_A' \vdash A$ and $\Gamma; \Delta_B' \vdash B$,
  %with $\Delta_A', \Delta_B' = \Delta$.
...
\begin{definition}
  Let $\instPD$ denote the following skew semiring on the partially ordered set
...
\begin{proof}
  ...
  \begin{mathpar}\footnotesize
    ...
    \inferrule*[right=$\tensor{}{}$-I]
    {
      ...
      \\
      \inferrule*{ }{(\rescomment 1) + (\rescomment 1) \subres (\rescomment 1)}
    }
...
admissibility of renaming, subusaging and substitution to the
...
for stating and proving these results. Though we have not had space to
elaborate here, \name{} is capable of representing several well known
linear and modal type theories by choice of semiring.
...
We plan to build on our work to generalise the framework of Allais
