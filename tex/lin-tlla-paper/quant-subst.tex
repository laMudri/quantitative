...
given a variable annotated $\pi$, we can coerce to treat it as if it has the
desired annotation $\rho$.
...
\begin{example}
  ...
  equivalent to Barber's DILL is the $0 \triangleright \omega \triangleleft 1$
...
$ A,B,C ::= \iota \mid A \multimap B \mid 1 \mid A \otimes B \mid 0
\mid A \oplus B \mid \top \mid A \with B \mid \excl{\rho} A$.  We have
a base type $\iota$, function types $A \multimap B$, tensor product
types $A \otimes B$ with unit $1$, sum types $A \oplus B$ with unit
$0$, ``with'' product types $A \with B$ with unit $\top$, and an
exponential $\excl{\rho} A$ indexed by a usage $\rescomment{\rho}$.
...
will sometimes silently cast between the types $\mathrm{Idx}~m$ and
...
If we instantiate $\mathbf R$ to be the $0 \triangleright \omega \triangleleft 1$ semiring, then we obtain a system that is equivalent to Barber's DILL \cite{Barber1996}.

For the sake of this paper, the precise rules chosen here are not too important.
...
Linear Logic \cite{Barber1996} and the modal system of Pfenning and Davis
