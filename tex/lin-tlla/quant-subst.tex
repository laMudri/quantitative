...
In future work, we plan to develop an equational theory for \name{},
and using that prove strong equivalence results between existing
systems and instances of our system.  We are working on generalising
the generic syntax with binding framework of Allais et
al. \cite{AACMM20} to work with usage annotations, which would allow
us to prove admissible rules for an even wider class of substructural
calculi.  We would like to compare the results we achieve here with
the framework of Licata, Shulman, and Riley \cite{LicataSR17}.
