...
omit the word ``left''.
A mnemonic for (left) skew semirings is ``multiplication respects operators on
the left from left to right, and respects operators on the right from right to
left''.
One may also describe multiplication as ``respecting'' and ``corespecting''
operators on the left and right, respectively.

From a skew semiring $\mathbf R$, we can form finite vectors, which we notate
as either $\mathrm{Vec}~n$ or $\mathbf R^n$.
Where an element of $\mathbf R$ describes how a \emph{variable} can be used, an
element of $\mathbf R^n$ describes how all of the variables in an $n$-length
\emph{context} can be used.
Therefore, we call such vectors \emph{usage contexts}, and take them to be row
vectors.
We use the following operations on usage contexts.

\begin{definition}[Vector operations]
  \begin{mathpar}
    \begin{matrix*}[l]
      {\subres} : \mathrm{Vec}~n \times \mathrm{Vec}~n \to \Omega \\
    ...
    \and
    \begin{matrix*}[l]
      + : \mathrm{Vec}~n \times \mathrm{Vec}~n \to \mathrm{Vec}~n \\
      (u + v)_i := u_i + v_i
    \end{matrix*}
    \and
    \begin{matrix*}[l]
      * : \mathbf R \times \mathrm{Vec}~n \to \mathrm{Vec}~n \\
      (\pi v)_i := \pi v_i
    \end{matrix*}
    \and
    \begin{matrix*}[l]
      \langle {-} \rvert : \mathrm{Idx}~n \to \mathrm{Vec}~n \\
      \langle i \rvert_j :=
      \begin{cases}
        1, & \textrm{if }i = j \\
        0, & \textrm{otherwise} \\
...
semiring $\mathbf R$. To highlight how usage annotations are used
throughout our development, we write all elements of $\mathbf R$, and
vectors and matrices thereof, in \rescomment{green}. This section
corresponds to the Agda module \AgdaModule{Specific.Syntax}.
...
a context with a specified type, and \emph{usage-cheked} variables,
...
\begin{definition}[Matrix operations]
  \begin{mathpar}
    \begin{matrix*}[l]
      {\subres} : \mathrm{Mat}~m~n \times \mathrm{Mat}~m~n \to \Omega \\
      M \subres N := \forall i j.~M_{ij} \subres N_{ij}
    \end{matrix*}
    \and
    \begin{matrix*}[l]
      \mat 0 : \mathrm{Mat}~m~n \\
      \mat 0_{ij} := 0
    \end{matrix*}
    \and
    \begin{matrix*}[l]
